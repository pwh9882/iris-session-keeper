...
- 툴바 아이콘 클릭 → 팝업에서 켜기/끄기, 서버 세션 상태, 남은 세션 시간, 마지막·다음 갱신 시각, **살린 시간**(세션 유지가 확인된 시점부터 실시간으로 올라가는 카운터 — 만료되거나 끄면 리셋) 확인, 즉시 갱신 가능.
- "포털 타이머"는 열려 있는 업무포털 탭의 30분 타이머 기준이라 업무포털 탭이 없으면 "포털 탭 없음"으로 표시됩니다. 서버 세션과는 별개입니다. 참고로 **로그아웃 상태에서 업무포털 주소를 열면 홈페이지로 조용히 리다이렉트**되므로, 로그인부터 해야 포털 탭으로 인식됩니다.
